...
<div class="max-w-5xl mx-auto p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white shadow rounded-lg p-5 text-center border">
            <img src="<?= base_url('uploads/' . esc($user['foto'] ?? 'default.png')) ?>" alt="Foto Profil" class="w-32 h-32 mx-auto rounded-full object-cover mb-4 border-4 border-blue-500 shadow-md">
            <h3 class="text-lg font-bold text-gray-800"><?= esc($user['nama']) ?></h3>
            <p class="text-sm text-gray-600"><?= esc($user['email']) ?></p>
            <p class="text-sm text-gray-600 mt-1"><?= esc($user['nohp']) ?></p>
        </div>

        <!-- Form Edit -->
        <form action="<?= base_url('profil/update') ?>" method="post" enctype="multipart/form-data" class="md:col-span-2 bg-white border rounded-lg shadow p-6 space-y-4">
            <?= csrf_field() ?>

            <!-- Ganti Foto -->
            <div>
                <label class="block text-sm font-medium text-gray-700">Ganti Foto</label>
                <input type="file" name="foto" class="mt-1 block w-full text-sm file:px-3 file:py-1 file:border file:border-gray-300 file:rounded">
            </div>

            <!-- Nama -->
            <div>
                <label class="block text-sm font-medium text-gray-700">Nama Lengkap</label>
                <input type="text" name="nama" value="<?= esc($user['nama']) ?>" class="mt-1 w-full px-4 py-2 border rounded text-gray-800" required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="nohp" value="<?= esc($user['nohp']) ?>" class="mt-1 w-full px-4 py-2 border rounded text-gray-800" required>
            </div>
